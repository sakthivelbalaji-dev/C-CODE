...
STUDENT_METADATA: dict[str, dict[str, str]] = {
    ...
    "add two numbers": {
        "constraints": "The user enters two ordinary integers. Formula: sum = a + b.",
    ...
    "check even/odd": {
        "constraints": "One integer (can be negative or zero). Formula: even iff (n % 2) == 0.",
        "algorithm": "If remainder modulo 2 is 0 → Even; else → Odd.",
        "functions": "Modulus `%`; `scanf` `%d`; `printf` literal words Even / Odd.",
    },
    "find largest of 3 numbers": {
        "constraints": "Three integers separated on one line (or consecutive reads). Formula: answer = max(a, b, c).",
    ...
    "print multiplication table": {
        "constraints": "One positive integer controlling the factor. Formula: k-th value = n * k for k = 1..10.",
    ...
    "factorial of number": {
        "constraints": "Small non-negative integer n (factorial grows fast). Formula: n! = 1*2*...*n; define 0! = 1.",
    ...
    "find sum of array": {
        "constraints": "First n, then exactly n integers on the next input line. Formula: sum = a[0] + a[1] + ... + a[n-1].",
    ...
    "largest element": {
        "constraints": "n then n integers. Formula: max = max(a[0], ..., a[n-1]).",
        "algorithm": "Track current maximum while scanning the array.",
        "functions": "`for` scan; conditional updates; `%d`; print winner.",
    },
    "matrix addition": {
        "constraints": "Dimensions plus two matrices stacked in stdin order. Formula: C[i][j] = A[i][j] + B[i][j].",
    ...
    "factorial using function": {
        "constraints": "Same as factorial puzzle but enforce your own helper. Formula: n! = 1*2*...*n; 0! = 1.",
        "algorithm": "Define `fact(n)` recursion or loop; print `fact(read_n)`.",
        "functions": "User-defined function; prototype + definition; `%d`.",
    },
    "fibonacci series": {
        "constraints": "Integer n = how many Fibonacci numbers to print; n may be 0 (print nothing). Formula: F(0)=0, F(1)=1, F(i)=F(i-1)+F(i-2) for i>=2.",
    ...
    "sum using recursion": {
        "constraints": "One positive integer n. Formula: S(n)=1+2+...+n = n*(n+1)/2; recursion S(n)=n+S(n-1) with S(1)=1.",
    ...
    "calculator switch menu": {
        "constraints": "Operator letter then two operands lines. Formula: + a+b; - a-b; * a*b; / integer division a/b (truncate toward zero for C-style ints).",
    ...
    "prime check": {
        "constraints": "Any integer: n<2 and negatives are Not; primes are ≥2 with no nontrivial divisor. Formula: n is prime iff n>=2 and for every integer d with 2<=d*d<=n, (n % d) != 0.",
    ...
    "leap year": {
        "constraints": "One integer Gregorian year (e.g. 1900, 1996, 2000, 2100). Formula: leap iff (year%400==0) OR (year%4==0 AND year%100!=0).",
    ...
    "simple interest": {
        "constraints": "Three integers Principal P, rate R, time T. Formula: SI = (P*R*T)/100 using integer division (truncate toward zero for nonnegative inputs).",
    ...
    "print primes in range": {
        "constraints": "Two bounds lo and hi on one line (space-separated), inclusive range. Formula: x is prime iff x>=2 and no d in [2, floor(sqrt(x))] divides x.",
    ...
    "armstrong number check": {
        "constraints": "Generalized Armstrong (narcissistic). Formula: let k = number of decimal digits of n; n is Armstrong iff n = d_1^k + d_2^k + ... + d_k^k where each d_i is a digit.",
    ...
    "gcd euclidean": {
        "constraints": "Two nonnegative integers on one line (space-separated). Formula: gcd(a,b)=gcd(b, a mod b) until b=0; then gcd(a,0)=a (checker defines gcd(0,0)=0).",
        "algorithm": "`while(b){ r=a%b;a=b;b=r;} ` classic swap.",
        "functions": "`%` loop; iterative not necessarily recursion.",
    },
    "lcm using gcd": {
        "constraints": "Two integers on one line (space-separated). Formula: lcm(a,b) = a / gcd(a,b) * b (integer-safe order with long long); equivalently |a*b|/gcd when it fits.",
    ...
    "max min average array": {
        "constraints": "`n` then n ints manageable array sizes. Formula: avg = floor((sum of all elements) / n) using integer division toward zero for nonnegative sums.",
    ...
    "second largest array": {
        "constraints": "At least length two ints; duplicates allowed. Formula: M = max(arr); if some element < M, answer = max{ x in arr | x < M }; else answer = M (all equal).",
    ...
    "linear search array": {
        "constraints": "Array listing plus target finder. Formula: return smallest index i with arr[i]==target, or -1 if none.",
    ...
    "matrix row column sums": {
        "constraints": "`3 × 3` numeric grid unchanged. Formula: row i sum = sum over j of A[i][j]; column j sum = sum over i of A[i][j].",
    ...
    "matrix transpose": {
        "constraints": "`3 × 3` square matrix. Formula: B[j][i] = A[i][j] for all i,j.",
        "algorithm": "`out[j][i]=in[i][j]` mapping.",
        "functions": "`i j` swaps indexes printing three lines.",
    },
    "symmetric matrix check": {
        "constraints": "`3 × 3` ints. Formula: symmetric iff A[i][j] == A[j][i] for every i,j.",
    ...
    "count above average": {
        "constraints": "Array ints general distribution. Formula: avg = floor(sum/n); count elements strictly greater than avg.",
    ...
    "pascal triangle rows": {
        "constraints": "`n` rows modest combinator growth. Formula: binomial C(n,0)=C(n,n)=1; C(n,k)=C(n-1,k-1)+C(n-1,k) for 0<k<n.",
    ...
    "bubble sort array": {
        "constraints": "`n` moderate sortable ints. Formula (bubble pass): if a[j] > a[j+1], swap them; repeat passes until no swaps.",
    ...
    "palindrome number": {
        "constraints": "Integer decimal digit symmetry checking. Formula: palindrome iff n equals the integer formed by reversing its decimal digits (ignore leading zeros except the lone 0 case).",
        "algorithm": "`reverse_digits` accumulator compare originals.",
        "functions": "`while n>0 remainder build reversed`.",
    },
    "strong number check": {
        "constraints": "Integer moderate digit factorial property. Formula: strong iff n = sum over each decimal digit d of (d!).",
